...
from tested.fib_summ_even_again_package.fib_sum_even import fib_sum_even_func
import doctest
...
class FibSumTest(TestCase):
    def test_fib_works(self):
        self.assertEqual(fib_sum_even_func(previous=0, next_=1), 4613732)

    def test_works_odd(self):
        self.assertEqual(fib_sum_even_func(previous=1, next_=5), 16019504)

    def test_works_even(self):
        self.assertEqual(fib_sum_even_func(previous=8, next_=10), 4749426)

    def test_zeros_both(self):
        with self.assertRaises(TypeError) as e:
            fib_sum_even_func(previous=0, next_=0)
