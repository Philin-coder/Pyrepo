...
class LcTest(TestCase):
    def test_is_list_return(self):
        self.assertTrue(isinstance(list((i * 3 if i % 2 else i * 2 for i in [i for i in range(1, 11)])), list) == True)

    def test_is_list_in(self):
        self.assertTrue(isinstance([i for i in range(1, 11)], list) == True)
