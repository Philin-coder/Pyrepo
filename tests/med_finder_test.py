...
class MedFinderTest(TestCase):
    def test_med_filer_works(self):
        self.assertEqual(median_finder(n=11), 6.5)

    def test_med_filer_works_ints_in_input(self):
        self.assertEqual(median_finder(n=10), 5)

    def test_med_filer_works_char_in_input(self):
        with self.assertRaises(TypeError) as e:
            median_finder(n='10')
        self.assertEqual('Введите положительное, целое число не равное 0', e.exception.args[0])

    def test_med_filer_works_negative_in_input(self):
        with self.assertRaises(TypeError) as e:
            median_finder(n=-10)
        self.assertEqual('Введите положительное, целое число не равное 0', e.exception.args[0])

    def test_med_filer_works_zero_in_input(self):
        with self.assertRaises(TypeError) as e:
            median_finder(n=-0)
        self.assertEqual('Введите положительное, целое число не равное 0', e.exception.args[0])
